...
def test_create_shared_data_instance_view(new_user, shared_data_form_data, request_factory):
    view = CreateSharedDataInstance.as_view()
    request = request_factory.post(path=reverse('files:upload-data'), data=shared_data_form_data)     
...
def test_get_shared_data_instance(new_shared_data, request_factory):        
    ...
    request = request_factory.get(path=reverse('files:get-data-instance'), data={"code": new_shared_data.code})
