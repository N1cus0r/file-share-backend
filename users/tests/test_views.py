...
from users.views import GetUserInfo, GetEditProfileUID, EditProfile, ActivateUserAccount, PerformPasswordReset


@pytest.mark.django_db
def test_create_user_view(create_user_form_data, api_client):
    response = api_client.post(path=reverse("users:create-user"), data=create_user_form_data)
...
def test_activate_account_view(encoded_uid, request_factory):
    ...
    request = request_factory.patch(reverse('users:activate-account'), data={'uid': encoded_uid})
...
def test_edit_profile_view(new_user, request_factory, edit_user_form_data):
    ...
    request = request_factory.patch(reverse('users:edit-profile'), data=edit_user_form_data)
    force_authenticate(request, user=new_user)
...
def test_perform_password_reset_view(request_factory, password_reset_data):
    ...
    request = request_factory.patch(reverse('users:perform-password-reset'), data=password_reset_data)         
